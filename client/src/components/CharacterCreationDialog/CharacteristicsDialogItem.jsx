        ...
        <Box>
          ...
          {showHighSpec && (
            <Box sx={{ mt: 1, display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: 'wrap' }}>
              {highExamples.length > 0 && (
                <Box sx={{ flex: '1 1 auto', minWidth: 200 }}>
                  <Typography variant="caption" color="success.main" fontWeight="medium" display="block" sx={{ mb: 0.5 }}>
                  ...
                  {highExamples.map((example, idx) => (
                    <Typography 
                      ...
                      sx={{ 
                        ml: 1, 
                        cursor: 'pointer',
                        '&:hover': { textDecoration: 'underline', color: 'success.main' }
                      }}
                      onClick={() => onSpecializationChange(`${char.key}_high`, example)}
                    >
                      • {example}
                    </Typography>
                  ))}
              ...
              <TextField
                ...
                onChange={(e) => onSpecializationChange(`${char.key}_high`, e.target.value)}
                size="small"
          ...
          {showLowSpec && (
            <Box sx={{ mt: 1, display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: 'wrap' }}>
              {lowExamples.length > 0 && (
                <Box sx={{ flex: '1 1 auto', minWidth: 200 }}>
                  <Typography variant="caption" color="error.main" fontWeight="medium" display="block" sx={{ mb: 0.5 }}>
                  ...
                  {lowExamples.map((example, idx) => (
                    <Typography 
                      ...
                      sx={{ 
                        ml: 1, 
                        cursor: 'pointer',
                        '&:hover': { textDecoration: 'underline', color: 'error.main' }
                      }}
                      onClick={() => onSpecializationChange(`${char.key}_low`, example)}
                    >
                      • {example}
                    </Typography>
                  ))}
              ...
              <TextField
                ...
                onChange={(e) => onSpecializationChange(`${char.key}_low`, e.target.value)}
                size="small"
                sx={{ flex: '0 0 auto', minWidth: 200 }}
              />
            </Box>
